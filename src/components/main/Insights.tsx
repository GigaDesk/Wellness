import * as React from "react";

export default function Insights() {
  return (
    <div
      style={{ fontFamily: "Graphik" }}
      className="text-xl font-normal pt-8 grid"
    >
      African SMEs are the backbone of our economies. However, they often face
      significant challenges, such as access to finance, infrastructure, and
      skilled labor. Our insights will explore these challenges and offer
      practical solutions to help your business thrive. Stay tuned for more
      in-depth analyses and actionable advice.
    </div>
  );
}
